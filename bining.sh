...
echo "Creating new conda environment named SqueezeMeta"
        ...
        conda create -y --name SqueezeMeta -c conda-forge -c bioconda -c fpusan squeezemeta
        conda install -y -c bioconda comparem
...
-p $binning/${project} \
-f ${host_removal} \
-extassembly ${Assembly}/all_reads/co-assembly.contigs.fa  2>&1 | tee $binning/log.txt
