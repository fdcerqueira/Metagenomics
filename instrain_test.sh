...
if [ -f "$SCRIPT_DIR/4_resident.fa" ] && [ -f "$SCRIPT_DIR/1_invader.fa" ]
  then
    ...
    akker=$SCRIPT_DIR/Akkermansia_muciniphila.fa
fi
...
#if [ ! -z "$resident" ] && [ ! -z "$invader" ] && [ ! -z "$akker" ]
#  then
#    echo "copying E.coli genomes Inv/Res"
...
filename=$(basename $dereplicated/dereplicated_genomes/4_resident.fa)
filename1=${filename%%.fa}
filename2=`echo ${filename1} | tail -c5`
seqkit replace -p ".+" -r "${filename2}_{nr}" $dereplicated/dereplicated_genomes/4_resident.fa > $dereplicated/${filename1}.rename.fa


filename=$(basename $dereplicated/dereplicated_genomes/1_invader.fa)
filename1=${filename%%.fa}
filename2=`echo ${filename1} | tail -c5`
seqkit replace -p ".+" -r "${filename2}_{nr}" $dereplicated/dereplicated_genomes/1_invader.fa > $dereplicated/${filename1}.rename.fa

#filename=$(basename $dereplicated/Akkermansia_muciniphila.fa)
#filename1=${filename%%.fa}
#filename2=`echo ${filename1} | tail -c5`
#seqkit replace -p ".+" -r "${filename2}_{nr}" $dereplicated/Akkermansia_muciniphila.fa > $dereplicated/${filename1}.rename.fa
